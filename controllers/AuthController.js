...
const delete_user = (req, res) => {
    ...
    UserModel.findOneAndUpdate({ _id: user_id }, { isApproved: false, archive: true })
        .then(response => {
...
const post_add_user = async (req, res) => {
    ...
    // fs.createReadStream('employee_dataset.csv')
    //     .pipe(csv())
    //     .on('data', (data) => {
...
const get_user_count = async (req, res) => {
    const user_count = await UserModel.countDocuments({archive: [false, null]})
...
const get_users = async (req, res) => {
    ...
    const Users = await UserModel.find({archive: [false, null]})
